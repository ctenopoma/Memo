...
erDiagram
    ...
    M365_USER ||--o{ SUBSTITUTE_SETTING : "設定する"
...
**在宅勤務申請リスト（APPLICATION）**

| 列名           | 型                             | 必須  | 説明                                                                  |
| -------------- | ------------------------------ | :---: | --------------------------------------------------------------------- |
| ID             | 自動採番                       |   ○   | リストID（PK・自動生成）                                              |
| Title          | 1行テキスト                    |   ○   | 申請タイトル（自動生成: 「{氏名}_{日付}」）                           |
...
| SubstituteUser | ユーザーまたはグループ |   ○   | 代理人（Person型）        |
| ValidFrom      | 日付のみ               |   ○   | 有効開始日                |
| ValidTo        | 日付のみ               |   ○   | 有効終了日                |
| Type           | 選択肢                 |   ○   | 種別（代理承認/代理申請） |
...
| No  | 画面ID          | 画面名                   | 概要                               | 利用者        |
| --- | --------------- | ------------------------ | ---------------------------------- | ------------- |
| 1   | SCR-TOP         | トップ（ダッシュボード） | 通知一覧、直近の申請・記録状況     | 全ユーザー    |
| 2   | SCR-APP-LIST    | 申請一覧                 | 自分の在宅勤務申請一覧             | 申請者        |
| 3   | SCR-APP-NEW     | 申請新規登録             | 在宅勤務の新規申請入力             | 申請者        |
| 4   | SCR-APP-DETAIL  | 申請詳細                 | 申請内容・承認状況の確認           | 申請者/承認者 |
| 5   | SCR-APP-EDIT    | 申請編集                 | 日付変更・再申請入力               | 申請者        |
| 6   | SCR-APR-LIST    | 承認待ち一覧             | 自分宛の承認待ち申請一覧           | 承認者        |
...
| ---- | ------------------ | ------------------------------------------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| A-01 | 承認依頼通知       | SharePointリスト「APPLICATION」にアイテムが作成された時             | 承認者にTeamsチャット・メールで承認依頼を送信                                 | 「Teamsにメッセージを投稿」「メールの送信(V2)」                                         |
| A-02 | 承認完了通知       | SharePointリスト「APPROVAL」にアイテムが作成された時（Action=承認） | 申請者にTeams・メールで承認完了を送信。写し先に在宅勤務情報を送信             | 「Teamsにメッセージを投稿」「メールの送信(V2)」                                         |
...
| A-09 | 代理承認自動切替   | スケジュール（毎日 午前0時）                                        | 代理設定の有効期間を確認し、期間外の設定を無効化                              | 「繰り返し(Recurrence)」→「複数のアイテムの取得」→「アイテムの更新」                    |
